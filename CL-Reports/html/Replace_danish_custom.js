//1) Remember to make a backup of the orginal, i recommend " bak.custom.js "
//2) replace the orginal index.html with this file, (rename it)
let playerID = null;
  ...
  $.post(`https://${GetParentResourceName()}/GetTopAdmins`, JSON.stringify({}), function(result) {
    if (result && result.length > 0) {
      ...
      result.forEach(function(admin, index) {
        ...
        var adminReports = $('<div>').addClass('admin-reports').text('med ' + admin.reports + ' rapporter denne uge.');

        adminContainer.append(adminIcon, adminName, adminReports);
        $('#leaderboard-list').append(adminContainer);
      });
    } else {
      var message = $('<div>').addClass('no-admin-message').text('Ingen top løver denne uge.');
...
function createReportItem(report) {
  ...
  if (report.reportReason === 'question') {
    reportedElement.textContent = 'Et spørgsmål';
  } else if (report.reportReason === 'bug') {
    reportedElement.textContent = 'En Fejl';
  } else if (report.reportReason === 'player') {
    reportedElement.textContent = 'En spiller';
  } else {
    reportedElement.textContent = report.reportReason;
  }
  
  const actionElement = document.createElement('button');
  actionElement.textContent = "Håndtér";
  ...
  getReportStatus(playerID, function(status) {
    if (status) {
      $.post(`https://${GetParentResourceName()}/SendNotify`, JSON.stringify({
        message: 'Du har allerede en aktiv rapport. Vent venligst, indtil din rapport bliver håndteret, eller når tiden udløber.',
        type: 'error',
      }));
    } else {
      const dropdown = document.querySelector('.dropdown-input');
...
toggleBtn.addEventListener('click', () => {
  ...
  if (toggleBtn.classList.contains('active')) {
    $.post(`https://${GetParentResourceName()}/SendNotify`, JSON.stringify({
      message: 'Rapportnotifikationer aktiveret. Du vil blive underrettet, når der indsendes nye rapporter.',
      type: 'success',
    }));
    ...
    $.post(`https://${GetParentResourceName()}/SendNotify`, JSON.stringify({
      message: 'Rapportnotifikationer deaktiveret. Du vil ikke blive underrettet om nye rapporter.',
      type: 'primary',
    }));
    $.post(`https://${GetParentResourceName()}/SetReceiveReports`, JSON.stringify({
